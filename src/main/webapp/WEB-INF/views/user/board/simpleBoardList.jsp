...
<script type="text/javascript">
		...
		for(var d=0;d<dataList.length;d++) {
			...
			while(true) {
				if(cursor.children.length > 0) {
					...
					while((cursor == null || cursor.parentElement == null || cursor.nextElementSibling == null) 
							&& cursor != target) {
				...
				if(cursor.className) {
					...
					for(var c=0;c<classList.length;c++) {
						...
						// replyForm 복사
						if((target.className == 'reply-list' && className == 'reply-modify')
								|| (target.className == 'board-list' && className == 'reply-write')) {
							var replyForm = document.getElementById('reply-form').cloneNode(true);
							
							replyForm.style.display = 'block';
							cursor.appendChild(replyForm);
						}
						...
						setNodeValue(value, cursor, className, fieldName, key, setClassList);
					}
				}
	...
	// 변수에 따라 노드 값 설정
	function setNodeValue(data, node, className, fieldName, key, setClassList) {
		var result = '';
		
		for(var i=0;i<setClassList.length;i++) {
			var setClass = setClassList[i];
			
			if(className == setClass) {
				return;
			}
		}
